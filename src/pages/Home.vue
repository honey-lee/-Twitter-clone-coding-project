<template>
  ...
  <div class="flex flex-col">
    ...
    <!-- tweeting section -->
    <div class="flex px-3 py-2 border-b-8 border-gray-100">
      <img :src="currentUser.profile_image_url" class="w-10 h-10 rounded-full hover:opacity-80 cursor-pointer" alt="">
      <div class="flex-1 flex flex-col ml-2">
        <textarea @keyup.enter="onAddTweet" v-model="tweetBody" placeholder="무슨 일이 일어나고 있나요?" class="w-full text-lg font-bold focus:outline-none mb-3 resize-none"></textarea>
        <div class="text-right">
          <button v-if="!tweetBody.length" class="bg-light text-white px-4 py-1 text-sm rounded-full font-bold">트윗</button>
          <button v-else @click="onAddTweet" class="bg-primary text-white px-4 py-1 text-sm rounded-full font-bold hover:bg-dark">트윗</button>
        </div>
      </div>
    </div>
    <!-- tweets -->
