      ...
      <div class="w-60 xl:1/4 pt-5 xl:ml-10 flex flex-col justify-between border-r border-gray-100">
        <div class="flex flex-col items-center xl:items-start">
          ...
          <div class="flex flex-col items-start">
            <router-link :to="route.path" :class="`hover:text-primary hover:bg-blue-50 px-4 py-2 rounded-full cursor-pointer ${router.currentRoute.value.name == route.name ? 'text-primary' : ''}`" v-for="route in routes" :key="route">
              <div v-if="route.meta.isMenu">
                <i :class="route.icon"></i><span class="ml-5 text-xl hidden xl:inline-block">{{ route.title }}</span>
              </div>
            </router-link>
          ...
          <div class="xl:w-full w-12 h-12 xl:pr-3 flex justify-center">
            <button @click="showTweetModal = true" class="mt-3 bg-primary text-white w-full h-12 rounded-full hover:bg-dark">
