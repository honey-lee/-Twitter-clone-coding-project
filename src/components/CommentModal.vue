    ...
    <div class="flex justify-center min-h-screen sm:pt-6 sm:px-4 sm:pb-20 text-center sm:block sm:p-0">
      ...
      <div @click.stop class="inline-block bg-white rounded-lg text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-lg w-full">
        ...
        <div class="text-right sm:hidden mr-2">
          <button v-if="!tweetBody.length" class="bg-light text-white px-4 py-1 text-sm rounded-full font-bold">답글</button>
          <button v-else @click="onAddTweet" class="bg-primary text-white px-4 py-1 text-sm rounded-full font-bold hover:bg-dark">답글</button>
        </div>
        </div>
        <div>
          <!-- original tweet -->
          <div class="flex px-4 pt-4 pb-3">
            <div class="flex flex-col">
              <img :src="tweet.profile_image_url" class="w-10 h-10 rounded-full hover:opacity-80 cursor-pointer" alt="">
          ...
          <div class="flex px-4 pb-4">
            ...
            <div class="flex-1 flex flex-col ml-2">
              <textarea @keyup.enter="onCommentTweet" rows="5" v-model="tweetBody" placeholder="내 답글을 트윗합니다." class="w-full text-lg font-bold focus:outline-none mb-3 resize-none"></textarea>
              <!-- tweet button -->
              <div class="text-right hidden sm:block">
                <button v-if="!tweetBody.length" class="bg-light text-white px-4 py-1 text-sm rounded-full font-bold">답글</button>
                <button v-else @click="onCommentTweet" class="bg-primary text-white px-4 py-1 text-sm rounded-full font-bold hover:bg-dark">답글</button>
              </div>
            </div>
          </div>
        </div>
